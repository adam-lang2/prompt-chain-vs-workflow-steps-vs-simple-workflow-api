...
agent; `GET_NEXT_STEP_TOOL`/`NextStepTool` (`prompt_chain_get_next_step.py`)
backs the prompt-chain agent and `BOOK_TENNIS_COURT_TOOL`
(`book_tennis_court.py`) backs the simple-workflow-api agent -- see each
module's own docstring for how its tool works.
"""
